...
export function SourceImageEvidence({review}:{review?:SourceImageReview}) {
  ...
  return <div className="mt-3 space-y-2 rounded-lg border border-white/25 p-3 print:border-gray-400">
    ...
    <p>{review.notes}</p>
    {review.capture && <><p>{validSourceURL(review.capture.image_url)&&<a className="underline" href={review.capture.image_url} target="_blank" rel="noopener noreferrer">Open image inspected ↗</a>}</p><p className="text-sm">Captured {new Date(review.capture.retrieved_at).toLocaleString('en-US',{timeZone:'America/New_York'})} Eastern. Referenced by the source page; this alone does not prove ownership.</p><details><summary className="cursor-pointer underline">Image evidence details</summary><p className="break-all text-xs">SHA-256: {review.capture.sha256}</p><p className="break-all text-xs">{review.capture.image_url}</p><p className="text-sm">The live image URL may change. The hash identifies the bytes inspected.</p></details></>}
