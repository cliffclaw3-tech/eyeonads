...
function SocialAd({ ad }: { ad: OfficeSocialAd }) {
  ...
  return <article className="min-w-0 space-y-4 rounded-xl border border-white/25 p-4 sm:p-5">
    ...
    <div className="space-y-3"><h4 className="font-semibold">Saved image evidence</h4>
      ...
      {observations ? <><p className="font-semibold">Partial ad coverage — saved image reviewed</p><p>{ad.image_review.notes}</p><p>Image type: {observations.creative_kind === 'complete_ad' ? 'ad creative image; surrounding post not included' : readable(observations.creative_kind)}. EHO logo or words: {readable(observations.eho)}. Brokerage identification: {readable(observations.brokerage)}. Contact details: {readable(observations.contact)}. License disclosure: {readable(observations.license)}.</p><p>{observations.notes}</p></> : <><p className="font-semibold">Image not reviewed</p><p>{ad.image_review.notes}</p></>}
...
export function OfficeSocialReport({ record }: { record: OfficeSocialRecord | null }) {
  ...
  return <section aria-labelledby="office-social-heading" className="my-8 min-w-0 space-y-4 rounded-2xl border border-white/25 p-4 sm:p-6">
    ...
    {evidence && <><div className="space-y-2"><p>Evidence saved: {date(evidence.retrieved_at)}</p><p className="break-words">Search: {evidence.query}</p>{source && <a href={source} target="_blank" rel="noopener noreferrer" className="inline-block underline">Open public Meta search ↗</a>}<p>{evidence.acquisition_note}</p>{evidence.acquisition_status !== 'available' && <p className="font-semibold">{evidence.acquisition_status === 'unavailable' ? 'Public ad evidence unavailable for this check.' : 'Only partial public ad evidence was captured.'}</p>}</div>
      {evidence.rotation && <div className="space-y-2 rounded-lg border border-white/25 p-4">
        <h3 className="font-semibold">Observed and assessed in this check</h3>
        <p>{evidence.rotation.observed} ad cards observed · {evidence.rotation.eligible} eligible for this office · {evidence.rotation.assessed_this_run} text assessments completed · {evidence.rotation.deferred} eligible cards deferred.</p>
        {evidence.rotation.selected_unassessed > 0 && <p>{evidence.rotation.selected_unassessed} selected cards did not complete text assessment and remain unchecked.</p>}
        {evidence.rotation.office_unverified > 0 && <p>{evidence.rotation.office_unverified} brokerage-reference cards were not assessed because their connection to this office or scoped roster was unverified.</p>}
        {evidence.rotation.excluded_scope > 0 && <p>{evidence.rotation.excluded_scope} cards were excluded from this office’s scope.</p>}
        <p className="text-sm">Metadata retained for {evidence.rotation.retained} known cards, including earlier observations. Retained or previously assessed does not mean currently visible, completely reviewed, or compliant. This is not a complete inventory of campaigns.</p>
      </div>}
      {!!evidence.inventory?.length && <details className="rounded-lg border border-white/25 p-4"><summary className="cursor-pointer font-semibold">Known ad metadata and deferred cards ({evidence.inventory.length})</summary><ul className="mt-3 space-y-3">{evidence.inventory.map(item=>{
        const observed=evidence.observed_cards?.find(card=>card.id===item.id);
        const attempted=evidence.ads.find(ad=>ad.library_id===item.id);
        const deferred=evidence.deferred_ids?.includes(item.id);
        const url=safeURL(item.url);
        return <li key={item.id} className="min-w-0 border-t border-white/20 pt-3"><p className="break-words"><strong>Meta ad {item.id}</strong> · {item.title}</p><p>{!observed?'Not observed in this check; historical metadata only.':observed.eligibility==='office_unverified'?'Office affiliation unverified — not assessed.':observed.eligibility==='out_of_scope'?'Outside this office scope — not assessed.':deferred?'Deferred by this check’s assessment budget.':attempted?.text_review_status==='reviewed'&&attempted.text_review?'Text assessment completed in this check.':'Text assessment did not complete in this check.'}</p>{observed&&<p className="text-sm">{observed.reason}</p>}<p className="text-sm">Last observed: {date(item.last_seen_at)}. {item.last_attempted_at&&<>Last attempt: {date(item.last_attempted_at)}. </>}Last successful text assessment: {item.last_assessed_at?date(item.last_assessed_at):'Never'}.</p>{item.content_changed_since_assessment&&<p className="text-sm">Observed content changed after its last successful text assessment.</p>}{url&&<a href={url} target="_blank" rel="noopener noreferrer" className="underline">Open original ad ↗</a>}</li>;
      })}</ul></details>}
      {evidence.ads.length ? <div className="space-y-5">{evidence.ads.slice(0, 4).map(ad => <SocialAd ad={ad} key={ad.library_id} />)}</div> : <p>No matching ad evidence was saved in this sample. This does not mean no public ads exist or that marketing is compliant.</p>}
