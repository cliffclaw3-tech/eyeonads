      ...
      {data && <>
        ...
        {agents.length > 0 && <section aria-labelledby="agents-heading" className="space-y-4">
          ...
          {visibleAgents.slice(0, visibleCount).map((agent) => {
            ...
            const status = active ? "Searching…" : review?.status === "complete" ? "Public search completed — review required" : review?.status === "failed" ? "Search failed — no completed assessment" : review?.status === "running" ? "Unfinished or in progress — refresh before retrying" : "Not searched";
