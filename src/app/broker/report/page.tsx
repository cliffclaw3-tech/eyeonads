    ...
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      {(setupResult.error||reviewsResult.error) ? <p role="alert">Report data could not be loaded. Refresh to retry; no coverage counts are confirmed.</p> : <>
        <section className="rounded-xl border border-white/25 p-5 print:border-gray-400">
          ...
          <p className="mt-3 text-sm">This screens sampled public ad text. Findings need human review and do not certify compliance.</p>
        ...
        <section><h2 className="text-2xl font-semibold">All agents</h2><ul className="mt-3 space-y-2">{rows.map(row=><li key={row.agent.id}><strong>{row.agent.name}</strong> — {row.reviewed.length} ad text assessment{row.reviewed.length===1?'':'s'}; {row.issues.length} needing review{row.saved?.searched_at?`; last search ${new Date(row.saved.searched_at).toLocaleDateString('en-US',{timeZone:'America/New_York'})}`:''}.</li>)}</ul></section>
