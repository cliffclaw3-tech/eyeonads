    ...
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      {(setupResult.error||reviewsResult.error) ? <p role="alert">Report data could not be loaded. Refresh to retry; no coverage counts are confirmed.</p> : <>
        <section className="rounded-xl border border-white/25 p-5 print:border-gray-400">
          ...
          {rows.some(row=>row.saved?.status==='running') && <p role="status" className="mt-3 rounded-lg border border-blue-300/50 p-3">Agent checks are running. Counts temporarily exclude earlier assessments for agents being rechecked and update as checks finish. <Link href="/broker/discovery" className="underline">View search progress</Link></p>}
