...
export async function GET(){
 ...
 return NextResponse.json({brokerage:setup.data?{name:setup.data.name,expected_agents:setup.data.expected_agents,website:setup.data.website,location:setup.data.location}:null,agents:setup.data?.agents??[],reviews:reviews.data??[]},{headers:{'Cache-Control':'no-store'}});
}
export async function POST(request:NextRequest){
 ...
 const agent=setup?.agents.find((a:{id:string})=>a.id===body.agent_id);
