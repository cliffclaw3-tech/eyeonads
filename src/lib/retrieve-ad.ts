...
export async function retrieveAd(ad:DiscoveredAd,agent:string,brokerage:string):Promise<DiscoveredAd & {selected_blocks?:{id:number;text:string}[];promotion_ids?:number[];identity_evidence?:{agent:string;brokerage:string};retrieval_status?:string;capture?:{url:string;sha256:string;retrieved_at:string;truncated:boolean}}> {
  ...
  try {
    ...
    return {...ad,page_access:capture?'page_read':'blocked',ad_text:'',capture,retrieval_status:stage,context:`${ad.context} ${capture?'Page retrieved, but exact ad text extraction was not verified.':'Direct page retrieval failed.'} Review the original source manually.`};
  }
}
